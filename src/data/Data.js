export const mainInfo = [
  'Tytuł projektu: “Rolniczy handel detaliczny, przetwórstwo oraz sprzedaż surowców rolnych konsumentom finalnym, poprzez tworzenie krótkiego łańcucha dostaw”.',
  'Projekt finansowany przez: Europejski Fundusz Rolny na rzecz Rozwoju Obszarów Wiejskich PROW 2014 – 2020 Działanie 16 „Współpraca” w przedmiocie operacji: tworzenie krótkich łańcuchów dostaw na zasadach płatności zryczałtowanej',
  'Wartość projektu 325 000,00 zł Środki EFRROW 206797,50 zł co stanowi 63,63% przyznanej pomocy.',
  'Krajowe środki publiczne 118202,50zł co stanowi 36,37% przyznanej pomocy.',
  'Okres realizacji operacji – 06.07.2022 r. do 05.07.2023 r.'
];

export const data = [
  {
    id: 1,
    logo: '/resources/hive.png',
    person: 'Artur Konopko',
    link: 'artur-konopko',
    place: 'Gospodarstwo Pszczelarskie "Patoka"',
    description:
      'Gospodarstwo pszczelarskie „Patoka” Artur Konopko powstało w 2011roku  z pasji do pszczół i miłości do przyrody. Moje podopieczne zbierają nektar oraz pyłek kwiatowy z terenów „zielonych płuc polski”, z serca puszczy knyszyńskiej jak również z terenów rolniczych wokół Białegostoku. Wielopniowa pasieka pozwala na zbiór miodów o zróżnicowanych walorach smakowych które są rozstawione w terenie o różnorodnej szacie roślinnej. Głównym celem jest dostarczenie moim odbiorcom obecnym jak i przyszłym, miodów wielokwiatowych (uważam że te są najlepsze w bogactwie swojego smaku) jak również miodów odmianowych z rzepaku, gryki, lipy, maliny. W ofercie sprzedaży posiadm również pyłek kwiatowy, pierzgę oraz propolis i wosk pszczeli. Wszystkim swoim odbiorcom życzę samych słodkich i pysznych dni.',
    phoneNumbers: ['+48794611132', '+48609596677'],
    email: 'artbi968@gmail.com',
    photos: [
      'https://i.imgur.com/tq1Qd99.jpg',
      'https://i.imgur.com/ywwLU08.jpg',
      'https://i.imgur.com/Gf6NDky.jpg',
      'https://i.imgur.com/98tdwa6.jpg',
      'https://i.imgur.com/CkBG0gW.jpg',
      'https://i.imgur.com/gbzZPPG.jpg'
    ]
  },
  {
    id: 2,
    logo: '/resources/barley.png',
    person: 'Marek Sosnowski',
    place: 'Gospodarstwo Rolne Sosnowscy',
    link: 'marek-sosnowski',
    description:
      'Jesteśmy rodzinnym gospodarstwem położonym na Podlasiu we wsi Nowodworce. Siedziba gospodarstwa znajduje się przy ulicy Wasilkowskie 28, przy tej samej ulicy są też tunele e których prowadzimy uprawę. \nGłówny profil produkcji to produkcja pod osłonami.\nW ramach Rolniczego Handlu Detalicznego oferujemy sadzonki ziół, takie jak bazylia, oregano, tymianek czy klika odmian mięty.\nNasza oferta obejmuje też ogórek gruntowy który doskonale nadaje się do przetworów domowych takich jak sałatki czy kiszenie.\nRównolegle prowadzimy też produkcję sadzonek kwiatów balkonowych i rabatowych, zaczynając od wiosennych bratków kończąc na jesiennej chryzantemie.',
    phoneNumbers: ['+48791100461'],
    email: 'msosnowski71@wp.pl',
    photos: [
      'https://i.imgur.com/pnaL534.jpg',
      'https://i.imgur.com/H0dvcNs.jpg',
      'https://i.imgur.com/dR67K6G.jpg'
    ]
  },
  {
    id: 3,
    person: 'Wioletta Januszkiewicz',
    logo: '/resources/hive.png',
    place: 'Gospodarstwo pszczelarskie',
    link: 'wioletta-januszkiewicz',
    description:
      'Gospodarstwo położone jest na terenie Podlasia w gminie Kuźnica we wsi Wojnowce. Pasieke liczącą 30 rodzin pszczelich prowadzę od 10 lat.\nPszczoły pracują z dala od miasta i przemysłu w naturalnym środowisku wśród łąk lasów i ogrodów.w mojej ofercie posiadam miód propolis pyłek pszczeli i wosk.\nPasieka zarejestrowana jest u lekarza weterynarii.\nZapraszam po nasze produkty. ',
    phoneNumbers: ['+48502938210'],
    email: 'violeta774@interia.pl',
    photos: []
  },
  {
    id: 4,
    person: 'Katarzyna Kożuchowska',
    link: 'katarzyna-kozuchowska',
    logo: '/resources/tomatoes.png',
    place: 'Gospodarstwo rolne we wsi Kolonia Mostowlany',
    description:
      'Gospodarstwo rolne we wsi Kolonia Mostowlany znajduje się na terenach położonych wśród malowniczych pól i lasów. Gospodarstwo zajmuje się przydomową uprawą pomidorów  z których powstaje przecier pomidorowy idealny do zup i sosów. Pomidory dojrzewają w pełnym słońcu na terenach czystych ekologicznie z dala od cywilizacji.',
    phoneNumbers: ['+48505033332'],
    email: 'katarzynakozuchowska11@wp.pl',
    photos: []
  },
  {
    id: 5,
    logo: '/resources/hive.png',
    person: 'Artur Błaszczuk',
    link: 'artur-blaszczyk',
    place: 'Pszczelarstwo',
    description:
      'Pszczelarstwo jest moją pasją od 14 lat. Zawdzięczam ją swojemu dziadkowi, także pszczelarzowi, który pozwalał mi obserwować pszczoły z bliska, podsuwał fachową literaturę, on też podarował mi swój pierwszy ul. Od tego momentu zaczęła się moja przygoda. Obecnie mam pasieki w trzech miejscach na Podlasiu, wszystkie umiejscowione są w ekologicznym otoczeniu, z dala od miast i obiektów przemysłowych, jedna z nich znajduje się w lesie.  W swojej ofercie posiadam różnorodne miody: mniszkowy, malinowo–akacjowy, lipowo–gryczany, spadziowy. Ponadto oferuję do sprzedaży pyłek kwiatowy, wosk i kit pszczeli.  Wielu klientów docenia smak i walory zdrowotne moich miodów i wraca po produkty w każdym następnym sezonie',
    phoneNumbers: ['+48603840638'],
    email: 'artur13.blaszczuk@gmail.com',
    photos: [
      'https://i.imgur.com/ZCBR3Ts.jpg',
      'https://i.imgur.com/qDsp0ge.jpg',
      'https://i.imgur.com/WbqvAzG.jpg',
      'https://i.imgur.com/IWQHvqQ.jpg',
      'https://i.imgur.com/TvqkEDU.jpg'
    ]
  }
];
